...
let grant;
let updates = [];
const db = { space: { findUnique: async () => space, update: async data => { updates.push(data); return space; } } };
const auth = { api: { getSession: async () => session } };
class AccessToken { constructor(_, __, options) { this.options = options; } addGrant(value) { grant = value; } async toJwt() { return 'test-token'; } }
let participantUpdate;
class RoomServiceClient { async getParticipant() { return { metadata: '{}' }; } async updateParticipant(room, identity, options) { participantUpdate = { room, identity, options }; } }
const mocks = { '@/lib/auth': { auth }, '@/lib/prisma': { prisma: db }, '@/lib/spaceAccess': { canAccessSpace }, 'livekit-server-sdk': { AccessToken, RoomServiceClient } };
