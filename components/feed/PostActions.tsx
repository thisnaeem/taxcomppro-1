  ...
  return <>
    <div className="feed-social-bar">
      <div className="feed-social-actions">
        ...
        <button type="button" onClick={onComments} aria-label={`${commentCount} comments`}><Comment01Icon size={21} /><span>{commentCount || "Comment"}</span></button>
        <button type="button" onClick={openShare} aria-label="Share post"><Share01Icon size={21} /><span>Share</span></button>
      </div>
      {total > 0 && <button type="button" className="feed-reaction-summary" onClick={onShowReactions} aria-label={`${total} reactions. View who reacted.`}>{top.slice(0,3).map(item => <span key={item.type} title={`${item.label}: ${counts[item.type]}`} aria-hidden="true">{item.emoji}</span>)}</button>}
    </div>
