...
export default function NetworkBranding({slug, initial, onSaved}: {slug: string; initial: Branding; onSaved: (b: Branding) => void}) {
  ...
  return <section className="pn-settings-card"><h3><PaintBoardIcon size={22}/>Network appearance</h3><p>Choose your network highlight color, logo, and cover image.</p>
    ...
    <div className="np-color-presets">{["#65a832", "#ffbe24", "#3b82f6", "#8b5cf6", "#ec4899", "#14b8a6"].map(color => <button key={color} type="button" aria-label={`Use ${color} highlight`} aria-pressed={draft.accentColor === color} onClick={() => setDraft(previous => ({...previous, accentColor:color}))} style={{background:color}} />)}<button type="button" onClick={() => setDraft(previous => ({...previous, accentColor:"#65a832"}))}>Reset to green</button></div>
